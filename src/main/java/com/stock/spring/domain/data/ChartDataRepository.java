...
public interface ChartDataRepository extends JpaRepository<ChartData, String> {
    ...
    @Query(nativeQuery = true,value = "SELECT * FROM chart_data WHERE company_name=?1 ORDER BY date LIMIT 1")
    ChartData dateStartByCompany(String company);

    @Query(nativeQuery = true,value = "SELECT * FROM chart_data WHERE company_name=?1 ORDER BY date DESC LIMIT 1")
